...
- mutation floor strategy: DECIDED floor-now-at-honest-baseline plus an explicit hardening follow-up. The per-package floors are `floor(measured)` of the firsthand 2026-06-24 baselines — admission 1.000→**1.0**, killswitch 0.839→**0.8**, quota 0.609→**0.6**, registry 0.529→**0.5** — and they ratchet UP only, never down. The honest-low suites (registry and quota) carry many surviving mutants; rather than silently ratcheting a weak floor in as "passing", they get a tracked HARDENING FOLLOW-UP — kill the surviving mutants, then raise the floor — as deliberate later work, and the phase is not blocked on hardening them. The floor makes regressions RED today; the follow-up deliberately raises the bar.
