...
- audit-chain-resume-across-restarts: DECIDED the OCSF hash-chain is ONE continuous per-source spine across daemon restarts (ADR-0009 detective posture: the chain detects deletion/truncation; component-07: the chain has zero breaks, order derives from the per-source monotonic sequence). At boot the sink RESUMES from the existing file's tip (`ReadTip` then `ResumeChainSink`) rather than re-anchoring at genesis (the prior bug, which wrote a second genesis segment that `ValidateChain` rejected). A decoupled tail (torn write, tamper) records an EXPLICIT chain-break marker — the only event allowed to re-anchor at genesis mid-file — and `ValidateChain` REJECTS a mid-file genesis re-anchor that carries NO marker, so a silent restart or a spliced segment is caught. `ValidateChain` now has shipped non-test callers: a boot-time whole-file verify (fail-closed — a file that already fails the chain aborts the boot) and `occ audit verify --file` for on-demand checks. The chain-break marker rides an omitempty `*ChainBreakInfo` pointer on the existing OCSF event, so a normal event's canonical bytes — and hash — are byte-identical to before the field existed (a guard test pins this).
- revoke-outcome-as-destroy-evidence: DECIDED the teardown finalizer's step-1 Storage-JWT revoke outcome (`marked_dead` / `already_dead` / `none_bound`) is recorded as an OPTIONAL evidence detail on a destroy record, NOT as a new privileged `audit.Action`. A revoke outcome is the result of a destroy whose `ActionDestroy` record was already emitted fail-closed on the lifecycle path — it is not a distinct operator action — so folding it into the closed SEC-45/SEC-72 Action set would dilute that spine. The outcome rides `metadata.unmapped.revoke_outcome` behind `omitempty`, mirroring the chain-break marker: a record that carries no outcome (every non-destroy event, and a destroy with no recorded outcome) is byte-identical to before the field existed, so no existing event's hash changes (a guard test pins this), and the closed Action enum and both exhaustiveness properties are untouched. The evidence record keys on the host-derived `SessionName` — the SAME value the primary `ActionDestroy` record keys on (lifecycle derives both the destroy key and the teardown Sandbox from `row.Key`) — so the evidence and its destroy event join on one `CorrelationUID`; the `FilesystemID` (the mount scope the revoke targeted) is kept as context, not the correlation key, so a regulated auditor never loses the join. An EMPTY value means the revoke step was not run or not recorded — semantically distinct from `none_bound` (a revoke that ran and bound nothing), which is never fabricated. The `RevokeAuditor` seam the docker finalizer already calls is wired in production by building the audit sink BEFORE the runtime provider (a boot reorder that leaves the kill-switch-first invariant untouched — that is the Boot sequencer's deny posture, gated on the listener-bind hook further down) and adapting the sink to the seam. A `none_bound` on a live destroy is an anomaly the evidence surfaces rather than masks: the auditor never skips the emit on `none_bound`. An evidence-emit failure is non-fatal — the destroy was already authorised fail-closed upstream and step-1 is idempotent, so a lost detail is logged, never escalated to a teardown abort that would strand the container.
- Deferred (audit prefix-integrity external anchor): the hash-chain proves PREFIX-integrity, not COMPLETENESS. A CLEAN truncation at a record boundary (deleting whole trailing records) leaves a still-valid prefix, which a resume continues from as though nothing were missing — resume-from-tip cannot detect it. Detecting a clean-boundary trailing-truncation needs an anchor OUTSIDE the file: a monotonically-advancing tip checkpoint (last sequence + hash) persisted in the control DB and compared at boot, or the downstream Merkle-head submission that seals each segment. Out of scope for v1 (the hot spine is short-lived until the cold-tier rotation seam lands); tracked as a follow-up. The chain still delivers the ADR-0009 minimal-shelf detective posture for every tamper class EXCEPT clean-boundary trailing-truncation.
- storage-jwt-exp-is-the-only-temporal-gate: DECIDED the Storage-JWT verifier gates on `exp` ONLY (`WithExpirationRequired`), and the `exp` edge is EXCLUSIVE — a token is valid strictly before `exp` (valid iff `now < exp`) and expired at or after it. `iat` is parsed into the claims for the audit trail but is NOT enforced as a not-before: a token whose issued-at is in the future relative to the verifying clock still verifies. This is intentional on the minimal shelf, not an oversight. (1) Control mints the weak Storage-JWT itself and never future-dates `iat`; a future `iat` could only come from a compromised minter, which would forge `exp`/scope too, so a not-before check buys nothing against the real threat. (2) The token is short-lived and `exp` is strictly gated, closing the only real replay window. (3) A hand-rolled not-before diverging from the verify library is its own risk on the crypto path. If the canon ever introduces `nbf`, that is an issuer-side ADR applied symmetrically to every consumer verifier (Control AND the sandbox), never a one-sided consumer patch. Pinned by `TestExpBoundaryIsExclusive` (the exclusive edge) and `TestIatIsInformationalNotGated` (a future `iat` still verifies), so a change that starts rejecting a future `iat` reds and forces a deliberate decision rather than a silent posture drift.
...
- exec-reply content ceiling is a source-side 64 KiB cap: DECIDED each stream of the buffered F5 exec reply is bounded to 64 KiB AT THE SOURCE (`defaultStdioCap`), with the `stdout_truncated`/`stderr_truncated` flag set when bytes are discarded (prefix-preserving). This is the caller-facing content ceiling, not merely a host-memory guard: it and the `ocu-mcp-gateway` reply read-cap are two independently-owned layers, and control's source cap is what keeps every reply under the far-side read-cap, retiring the large-output over-cap failure class. It is a const, not a deployment flag — a knob would let a deployment silently raise the cap past the gateway read-cap and recreate the failure class, and a same-boundary second truncation layer would be a dead knob (defense-in-depth only holds across independently-owned boundaries, which is exactly the control-source/gateway-read split). A control-side test mirrors the gateway read-cap by name and asserts the worst-case reply (both streams base64 at the ceiling plus envelope) stays under it; the boundary test pins that exactly 64 KiB is untruncated and one byte past it truncates. SCOPE: the buffered exec reply only, not any interactive streaming path. The frozen JSON field names are unchanged; the canon schema pinning the F5 exec-reply shape is a separate owner-gated follow-up (`open-computer-use#130`).
