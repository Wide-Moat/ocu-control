...
func activityFor(a audit.Action) (uint8, string) {
	...
	case audit.ActionDestroy, audit.ActionReconcileReclaim:
		...
		// like a destroy, tears a session down — it drives the row to the released
		// tombstone and returns its concurrency slot — so the honest CRUD slot is
		// Delete. The name stays the Action label ("reconcile_reclaim"), so the record
		// is self-describing next to the operator-driven destroy events.
