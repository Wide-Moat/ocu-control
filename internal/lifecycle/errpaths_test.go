...
func TestReconcileSkipsActiveRow(t *testing.T) {
	...
	// provider.Reconcile returns. The reconciler must then leave the ACTIVE row alone
	// — a live container matched to an active row is never killed and the row is not
	// reclaimed.
	d.provider.orphans = []runtime.Sandbox{{Name: runtime.SessionName(created.Key)}}
