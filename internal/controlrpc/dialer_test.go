...
// hostOwnedSockDir creates a 0700 directory the dialing process owns, mirroring
// the handoff stager's sock dir. It is rooted in a SHORT base (not the long nested
...
// TestDialRefusesNon0700Dir asserts a sock dir that is NOT 0700 host-owned (here,
// group/other-readable) is refused by the host-owned gate BEFORE any connect(2),
// even though a live listener is bound: a non-host peer's socket is dropped before
// any frame is parsed (the v1 host-only-at-accept realization).
func TestDialRefusesNon0700Dir(t *testing.T) {
	skipIfNoUDS(t)
	dir := shortDir(t, 0o755)
	sockPath := filepath.Join(dir, "control.sock")
	...
	if !errors.Is(err, controlrpc.ErrSockDirGate) {
		t.Fatalf("loose sock dir: want ErrSockDirGate, got %v", err)
	}
}
